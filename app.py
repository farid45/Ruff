...
def create_app():
    ...
    def sglistsingle():
        ...
        newseatnumdf.drop(
            columns=newseatnumdf.columns.difference(skycolstokeep),
            inplace=True,
        )
        ...
        if ownsizedf.empty:
            ...
            datadf.drop(
                columns=datadf.columns.difference(colstokeep),
                axis=1,
                inplace=True,
            )
            newdf = broaddf.merge(
                datadf,
                right_on=["SECTION_NAME"],
                left_on=["section_match"],
                how="left",
            )
            ...
            newdf.drop(
                newdf.loc[newdf["SECTION_NAME"] == "NA"].index,
                inplace=True,
            )
    ...
    def sgreport():
        ...
        newdf = datadf.merge(
            broaddf,
            left_on=["SECTION_NAME"],
            right_on=["section_match"],
            how="right",
        )
        newdf.fillna("NA", inplace=True)
        newdf.drop(
    ...
    def sgsoldinvupdate(inid: int):
        ...
        if token == tokenkey:
            ...
            print(
                solddf.loc[solddf["invoiceId"] == updated_invoice["invoiceId"]],
            )
    ...
    def trtmlbcounts():
        ...
        todaysecdf = todaydf.drop(
            todaydf.columns.difference(seccolstokeep),
            axis=1,
        )

        yesterdaysecdf = yesterdaydf.drop(
            yesterdaydf.columns.difference(seccolstokeep),
            axis=1,
        )
